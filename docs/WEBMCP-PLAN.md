...
| 51 | `move_audio` | consequential | `target` (a shot id for its VO, a cue id, or part of a file name), `at?` (film seconds), `delta?` (seconds to slide), `snap?` (default true: land on a cut or the playhead), `project?` | Timeline → the A1 / MUSIC / SFX lanes (`timeline.audio.drag`); the same move a drag makes | moved, track, from, to, snapped_to, `vo_offset` + `lines_moved` for VO or `cue` for a cue, and `overlaps` — what it still sits under, or "none". VO → the shot's `vo_offset` override; a cue → `POST /cues/{id}/move`, which keeps a shot-anchored cue anchored |
...
| 50 | `generate_takes` (still/restyle), image model registry | consequential | `model?` now also takes an image registry key: `"pro"` for readable text, `"flash"` (the cheap default), or a full OpenRouter id | Shot Editor → Generate → still/restyle; the model picker lists the registry with its measured price | the still lane's twin of the motion registry (`GET /api/image-models`): $/still, registers (`legible_text`, `typography`, `complex_composition`, `cheap_default`), failure modes, fallback. A prompt naming text, a sign, a screen or a title that ran on the cheap model returns `hint: 'This shot asks for readable text: consider model:"pro" (≈$0.14 per still)'`; `describe_shot` and `list_backends` list the registry with prices, and the take records the model id and its billed cost so `/spend` stays right |
