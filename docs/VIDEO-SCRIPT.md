# Demo video script: Genga Studio × WebMCP
...
**Client:** ChatGPT Desktop, in-app browser, Genga Studio open on the Two Claudes
...
**On screen:**
- 0:00 Chrome, Genga Studio on the Two Claudes Film Editor, DevTools already docked
...
- Keep the topbar tools chip in frame in Blocks A and D. It reads
  `tools: 39 · native` and it is the proof the API is live.
...
- Upload **public**, not unlisted. Title:
  `Genga Studio: an agent made a film through the page's own WebMCP tools`. Put the
