...
source of truth for every feature in the app, 23 tools published on
...
| `tools/**` | The 23-tool catalogue: `find.ts`, `navigate.ts`, `generate.ts`, `motion.ts`, `picks.ts`, `timing.ts`, `audio.ts`, `music.ts`, `direct.ts`, `film.ts`, `jobs.ts`, plus `deps.ts`, `util.ts`, `fakeContext.ts` and `descriptions.md`. |
...
The 23 tool names, in `contract.ts` order: `find_shots`, `describe_shot`,
