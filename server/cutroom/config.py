...
CUTROOM_ACCESS_FORM_URL   request-access form linked from the public page (empty = no button)
CUTROOM_DEMO_VIDEO_URL    YouTube walkthrough shown on the public page (empty = no embed)
...
class Settings(BaseSettings):
    ...
    # The public page (api/public.py). Both are links the owner sets on the
    # host, read per-process rather than baked into the SPA bundle, so setting
    # one is a variable change and not a rebuild.
    access_form_url: str = ""     # CUTROOM_ACCESS_FORM_URL — the Google Form
    demo_video_url: str = ""      # CUTROOM_DEMO_VIDEO_URL — the YouTube walkthrough
