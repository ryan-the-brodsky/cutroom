...
for r in rows:
    ...
    if not a.no_motion and r.get("motion_prompt"):
        steps += [
            ...
            {"tool": "wait_for_jobs", "args": {"jobs": "$prev.jobs", "timeout_s": 60}, "note": f"{sid} wait motion", "allowFail": True},
            {"tool": "wait_for_jobs", "args": {"jobs": "$prev.jobs", "timeout_s": 60}, "note": f"{sid} wait motion (2)", "allowFail": True},
